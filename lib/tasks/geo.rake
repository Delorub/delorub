namespace :geo do
  ...
  task import: :environment do
    ...
    csv.each do |row|
      ...
      next if PlaceTypeName.find_by_id row[:place_type_name_id]
      PlaceTypeName.create(id: row[:place_type_name_id],
        level: row[:level],
        code: row[:code],
        name: row[:name],
    ...
    ['public/geo/regions.csv', 'public/geo/places.csv', 'public/geo/streets.csv'].each do |name|
      ...
      csv.each do |row|
        row = row.to_hash.symbolize_keys
        next if Place.find_by_id row[:place_id]
        Place.create(id: row[:place_id],
